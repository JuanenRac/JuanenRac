...
`starter-kit.bat`）を実行すると、下記の表にある 12 個のリポジトリすべ
てを、1 つのディレクトリ内に兄弟フォルダとしてクローンできます——これ
は、本リポジトリ内のあらゆるクロスリポジトリスクリプトがすでに前提と
している標準的なディレクトリ構成です。再実行しても安全です（すでに
クローン済みのものには一切手を加えません）。その後は、
